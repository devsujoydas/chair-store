...
const Header = () => {
  ...
  return (
    <header className="fixed w-full z-20 bg-[#3B5D50] top-0 shadow shadow-[#2e2c2c] font-montserrat ">
      ...
      <div className="text-[#B4BECC]  max-w-7xl mx-auto flex flex-col md:flex-row gap-10 justify-between md:items-center  md:py-7 py-3">
        ...
        <div className="flex justify-between items-center px-10 ">
          ...
          <div className="relative overflow-hidden">
            <a href="/" className='flex items-center justify-center gap-2'>
              <h1 className='text-4xl text-white font-semibold'>Chair Store</h1>
            </a>
          </div>

          <div onClick={() => setHumbarger(!humbarger)} className="md:hidden block cursor-pointer text-5xl text-secondary shadow-2xl">
            <LuMenu />
          </div>
        </div>
        ...
        <div className={humbarger ? 'md:hidden fixed z-20 top-0 -left-121 w-full h-screen grid grid-cols-5 opacity-0 duration-700 transition-all' : 'md:hidden opacity-100 fixed z-20 top-0 left-0 w-full h-screen grid grid-cols-5 duration-700 transition-all'} >
          <div className=" col-span-4 px-8 bg-[#3B5D50] space-y-10">
            <div className="flex justify-between items-center py-5 ">
              ...
              <div onClick={() => setHumbarger(!humbarger)} className="md:hidden block text-4xl">
                <IoClose className="text-secondary cursor-pointer shadow-2xl shadow-gray-200 bg-[#548170] rounded-full p-1" />
              </div>
            </div>

            <hr className="text-gray-700 my-5" />
            <div onClick={() => setHumbarger(!humbarger)} className="flex flex-col text-xl gap-5">
              <Nav />
            </div>
            <hr className="text-gray-700 my-5" />
            <FindWithMe/>
